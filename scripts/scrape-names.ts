...
const JS_KEYWORDS = new Set([
  ...
  "public", "static", "yield", "await", "async", "of", "from", "as", "get",
  "set",
  // literals
...
function isValidString(s: string): boolean {
  ...
  if (!/^[a-zA-Z][a-zA-Z0-9._\-\s]*$/.test(s)) return false;
  // Skip strings that are just numbers
  if (/^\d+(\.\d+)?$/.test(s)) return false;
  // Skip hex-only strings (hashes, colors)
  if (/^[0-9a-fA-F]+$/.test(s) && s.length > 4) return false;
...
async function main() {
  ...
  for (const n of sortedNumbers) {
    if (finalNumbers.length >= TARGET_NUMBERS) break;
    if (!used.has(n)) finalNumbers.push(n);
  }
